"""Integration tests for MySQL connector — runs against a live MySQL container."""
...
class TestMySQLConnector:
    """Test MySQLConnector against a real MySQL instance."""

    @pytest.fixture(autouse=True)
    def setup(self, mysql_conn_info):
        from open_navicat.dal.connection_pool import _loop
        from open_navicat.dal.mysql_connector import MySQLConnector

        self.info = mysql_conn_info
        self.connector = MySQLConnector(mysql_conn_info)
    ...
    def test_execute_select(self):
        ...
        _loop.run_until_complete(self.connector.execute(
            "INSERT INTO t_select VALUES (1, 100), (2, 200) ON DUPLICATE KEY UPDATE val=VALUES(val)"
        ))
    ...
    def test_execute_insert_update_delete(self):
        ...
        # Insert
        _loop.run_until_complete(self.connector.execute(
            "INSERT INTO t_crud VALUES (1, 10) ON DUPLICATE KEY UPDATE val=VALUES(val)"
        ))
        # Update
        _loop.run_until_complete(self.connector.execute(
    ...
    def test_fetch_page(self):
        ...
        for i in range(10):
            _loop.run_until_complete(self.connector.execute(
                f"INSERT INTO t_page VALUES ({i}, {i*10}) ON DUPLICATE KEY UPDATE val=VALUES(val)"
            ))
