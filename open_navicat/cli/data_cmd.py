...
def profile_table(
    table: str = typer.Argument(..., help="Table to profile (format: db.table or conn:db.table)"),
    conn: str = typer.Option("", "--conn", "-c", help="Connection name (default: active)"),
    sample_size: int = typer.Option(10000, "--sample", "-s", help="Max rows to sample for profiling"),
) -> None:
    """Analyze column statistics and data distribution for a table."""
    def parse_table(spec: str) -> tuple[str, str, str]:
        parts = spec.split(":", 1)
        if len(parts) == 1:
            ...
            cid = _resolve_conn(parts[0])
            db, tbl = parts[1].split(".", 1)
        return cid, db, tbl

    cid, database, table_name = parse_table(table)
    q = "`"

    # 1. Get column metadata
    from open_navicat.dal.connection_pool import _loop as pool_loop, connection_pool
    from open_navicat.services.metadata_service import metadata_service

    if not connection_pool.get(cid):
        console.print("[red]Connection not open. Use 'conn open' first.[/red]")
        raise typer.Exit(1)

    info = metadata_service.get_table_info(cid, database, table_name)
    if not info or not info.columns:
        console.print("[red]Table not found or has no columns.[/red]")
        raise typer.Exit(1)

    console.print(f"[yellow]📊 Profiling {database}.{table_name} ({len(info.columns)} columns)...[/yellow]")

    connector = connection_pool.get(cid)

    # 2. Build a single profiling SQL query
    ...
    for col in info.columns:
        ...
        qn = f"{q}{name}{q}"
        is_numeric = dt in ("INT", "BIGINT", "SMALLINT", "TINYINT", "DECIMAL", "FLOAT", "DOUBLE", "NUMERIC", "REAL")
        is_temporal = dt in ("DATE", "DATETIME", "TIMESTAMP", "TIME", "YEAR")

        select_parts.append(f"COUNT({qn}) AS c_{name}")
        select_parts.append(f"COUNT(DISTINCT {qn}) AS d_{name}")
        select_parts.append(f"SUM(CASE WHEN {qn} IS NULL THEN 1 ELSE 0 END) AS n_{name}")

        if is_numeric:
            select_parts.append(f"MIN({qn}) AS min_{name}")
            select_parts.append(f"MAX({qn}) AS max_{name}")
            select_parts.append(f"AVG({qn}) AS avg_{name}")
            col_labels.append({
                "name": name, "type": "numeric", "data_type": col.data_type,
            })
        elif is_temporal:
            select_parts.append(f"MIN({qn}) AS min_{name}")
            select_parts.append(f"MAX({qn}) AS max_{name}")
            col_labels.append({
                "name": name, "type": "temporal", "data_type": col.data_type,
            })
        else:
            select_parts.append(f"MIN(LENGTH({qn})) AS minl_{name}")
            select_parts.append(f"MAX(LENGTH({qn})) AS maxl_{name}")
            col_labels.append({
                "name": name, "type": "string", "data_type": col.data_type,
            })

    sql = f"SELECT {', '.join(select_parts)} FROM {q}{database}{q}.{q}{table_name}{q}"
    sql += f" LIMIT {sample_size}" if sample_size else ""

    result = pool_loop.run_until_complete(connector.execute(sql))
    if not result or not result.rows:
        console.print("[red]Failed to profile table.[/red]")
        raise typer.Exit(1)
    ...
    for label in col_labels:
        name = label["name"]
        c = row[col_index.get(f"c_{name}", 0)]
        d = row[col_index.get(f"d_{name}", 0)]
        n = row[col_index.get(f"n_{name}", 0)]
        total = c or 0
        distinct = d or 0
        nulls = n or 0
        pct_null = f"{nulls / total * 100:.1f}%" if total else "-"
        pct_distinct = f"{distinct / total * 100:.1f}%" if total else "-"
        ...
        if label["type"] == "numeric":
            ...
            max_key = f"max_{name}"
            avg_key = f"avg_{name}"
            entry["最小值"] = row[col_index.get(min_key, 0)] if min_key in col_index else "-"
            entry["最大值"] = row[col_index.get(max_key, 0)] if max_key in col_index else "-"
            entry["平均值"] = round(row[col_index.get(avg_key, 0)], 4) if avg_key in col_index else "-"
        elif label["type"] == "temporal":
            min_key = f"min_{name}"
            max_key = f"max_{name}"
            entry["最早"] = str(row[col_index.get(min_key, 0)] or "-") if min_key in col_index else "-"
            entry["最晚"] = str(row[col_index.get(max_key, 0)] or "-") if max_key in col_index else "-"
        else:
            minl_key = f"minl_{name}"
            maxl_key = f"maxl_{name}"
            entry["最短长度"] = row[col_index.get(minl_key, 0)] if minl_key in col_index else "-"
            entry["最长长度"] = row[col_index.get(maxl_key, 0)] if maxl_key in col_index else "-"

        profile_rows.append(entry)

    format_output(profile_rows, "table", title=f"📊 Data Profile: {database}.{table_name}")
    console.print(f"\n[dim]Sampled up to {sample_size} rows. Use --sample to adjust.[/dim]")
